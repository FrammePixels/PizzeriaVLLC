...
import { ShoppingCart, Trash2, Tag, ChevronRight, Zap, Lock, AlertCircle } from 'lucide-react';

export default function NeonCart() {
  const { cartShop, removeItem, deleteItemCart, PriceFinal, priceAfterDiscount, applyDiscount } = useAuth();
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setGlowIndex(prev => (prev + 1) % cartShop.length);
    }, 2000);
  ...
  return (
    <div className="min-h-screen bg-gray-950 relative overflow-hidden">
      ...
      <div className="absolute bottom-0 right-1/4 w-[600px] h-[600px] bg-purple-500 rounded-full mix-blend-screen filter blur-3xl opacity-10 animate-pulse" style={{ animationDelay: '1s' }} />
      <div className="absolute top-1/2 left-1/2 w-[600px] h-[600px] bg-pink-500 rounded-full mix-blend-screen filter blur-3xl opacity-10 animate-pulse" style={{ animationDelay: '2s' }} />
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-lg font-bold text-white transition-all relative overflow-hidden group ${
                    cartShop.length === 0 
                      ...
                      : 'bg-gradient-to-r from-cyan-500 via-purple-500 via-pink-500 to-orange-500 hover:shadow-xl hover:shadow-purple-500/50'
                  }`}
                  style={cartShop.length > 0 ? { animation: 'float 3s ease-in-out infinite' } : {}}
                >
                  {cartShop.length === 0 ? (
